...
def run_checks():
    ...
    test_result = subprocess.run([sys.executable, '-m', 'unittest', 'discover', 'tests'], capture_output=False)
    ...
    linter_result = subprocess.run([sys.executable, '-m', 'pylint', 'src', '--disable=C0111,C0103'], capture_output=False)
